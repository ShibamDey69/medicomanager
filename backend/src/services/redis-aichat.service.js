...
export class MedicalAssistantController {
  constructor(apiKey, redisOptions = {}) {
    ...
    this.worker = new Worker("medical-assistant", async (job) => {
  ...
  createMedicalPrompt(userData, question) {
    ...
    const age = user.DOB ? Math.floor((new Date() - new Date(user.DOB)) / (365.25 * 24 * 60 * 60 * 1000)) : 'unknown';
    ...
    if (prescriptions.length > 0) {
      ...
      prescriptions.forEach((prescription, index) => {
        prescriptionsInfo += `${index + 1}. Doctor: ${prescription.doctor} (${prescription.specialty})\n`;
        prescriptionsInfo += `   Date: ${new Date(prescription.date).toLocaleDateString()}\n`;
        ...
        if (prescription.medicines && prescription.medicines.length > 0) {
          ...
          prescription.medicines.forEach((med, medIndex) => {
            prescriptionsInfo += `     - ${med.name || 'Unknown'} (${med.dosage || 'N/A'})\n`;
          });
        }
        prescriptionsInfo += "\n";
      });
...
`;
  ...
  async processMedicalQuery(userData, question) {
    try {
      ...
      throw new Error("Failed to get medical assistance");
    }
  }

  async submitQuery(userData, question) {
    const jobId = `medical_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    await this.queue.add("medical-query", {
      userData,
      question,
      jobId
    }, {
